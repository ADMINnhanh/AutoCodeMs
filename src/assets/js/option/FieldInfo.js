/**
 * @Description: 项目列表 -> 项目详情 -> 右侧 分页表格-字段信息 | 配置项
 * @Version: 1.0
 * @Author: xcl
 * @Date: 2022-10-31 09:51:01
 * @LastEditors: xcl
 * @LastEditTime: 2022-12-16 13:49:42
 * @FilePath: \web\src\assets\js\option\FieldInfo.js
...
export default (safe) => {
  ...
  return {
    ...
    columnBtn: false,
    dialogDrag: true,
    addTitle: "新增字段（t_solution）",
    editTitle: "编辑字段（t_solution）",
    viewTitle: "查看字段（t_solution）",
    updateBtnText: '保存',
    saveBtnText: '保存',
    cancelBtnText: '取消',
    // cellBtn: true,
    ...
    column: [
          ...
          },
          {
            validator: function (rule, value, callback) {
              rulesUtils.byAPIDuplicationName(
                ...
                {
                  url: `${safe.$common.basePath}/field/getFieldByTableId`,
                  method: "get",
                  params: {
                    tableId: parseInt(safe.$route.query.tableId),
                  },
                  hideLoading: true,
                },
              )
      ...
      },
      {
        ...
        control: (val, form) => {
          ...
          if (val) {
            // push 添加指定元素到数组中
            form.functionIds.push(6)
          } else {
            // Set 删除数组中的指定元素
            let newSet = new Set(form.functionIds)
            newSet.delete(6)
            form.functionIds = [...newSet]
          }
        },
      },
      {
        ...
        html: true,
        formatter: (val) => {
          return val.dictType ? val.dictType + "（" + val.dictName + "）" : ''
        }
      },
      {
        label: "字段默认值",
